...
TEST_CASE("GET_ALL_STATEMENTS_TEST") {
    // Init
    PKB db = PKB();
    STMT_NO actualList[1000];
    for (int i = 0; i < 1000; i++) {
        db.addStatement(i);
        actualList[i] = i;
    }

    REQUIRE(db.getAllStatements() == LIST_STMT_NO(actualList,
        actualList + sizeof actualList / sizeof actualList[0]));
}

TEST_CASE("GET_ALL_STATEMENTS_EMPTY_TEST") {
    // Init
    PKB db = PKB();
    REQUIRE(db.getAllStatements() == LIST_STMT_NO({}));
}
...
TEST_CASE("PKB_FOLLOW_API_TEST") {
    ...
    // Check relationship
    REQUIRE(db.getAllStatements() == LIST_STMT_NO({1, 2, 3, 4, 5}));
    for (int s1 = 1; s1 < 5; s1++) {
        REQUIRE(db.getFollows(s1) == s1 + 1);
        LIST_STMT_NO expected({});
        for (int s2 = s1; s2 < 5; s2++) {
            expected.push_back(s2 + 1);
